...
import utilities.numerical_methods as num_meth
...
for i in files:
    ...
    for line in g:
        ...
        t = Time(columns[0],format='isot',scale='utc')# - timedelta(seconds = 0.6) #+ timedelta(seconds = 0.5)
        utc_time.append(t)
        jd_time.append(t.jd)
    ...
    #+/- 15 mins...
    start_time_JD = min(JD_time_a) - (6*15/(24*60))
    end_time_JD = max(JD_time_a) + (6*15/(24*60))
...
#melb = [-37.680589141*u.deg, 145.061634327*u.deg, 115.083*u.m]
...
for i in time_stamp:
    ...
    if bb.ra/u.deg < 180:
        qzss_ra.append((bb.ra/u.deg) + 360.)
    else:
...
for i in JD_time_a:
    ...
    lagr_ra = num_meth.interp_lagrange(plot_time, qzss_ra_2, i, 9)
    diff_ra_lagr.append(RA_array_a[index] - lagr_ra)
    
    lagr_dec = num_meth.interp_lagrange(plot_time, qzss_dec_2, i, 9)
    diff_dec_lagr.append(Dec_array_a[index] - lagr_dec)
...
diff_ra_lagr = np.array(diff_ra_lagr)
diff_dec_lagr = np.array(diff_dec_lagr)


mean_diff_ra_lagr = np.nanmean(diff_ra_lagr)
mean_diff_dec_lagr = np.nanmean(diff_dec_lagr)
...
plt.text(0.5*(xmax-xmin)+xmin,0.7*(ymax-ymin)+ymin,"Mean: "+str(mean_diff_dec_2*3600)+" arcseconds",color = 'blue')
plt.text(0.5*(xmax-xmin)+xmin,0.8*(ymax-ymin)+ymin,"Mean: "+str(mean_diff_dec_lagr*3600)+" arcseconds",color = 'red')

#Declination
#plt.plot(JD_time_a,diff_dec,'ro',JD_time_a,diff_dec_2,'bx',JD_time_a,diff_dec_3,'r+',JD_time_a,diff_dec_lagr,'b+')
plt.plot(JD_time_a,diff_dec_2,'bx',JD_time_a,diff_dec_lagr,'r+', [xmin,xmax], [0,0],'--')
...
plt.text(0.5*(xmax-xmin)+xmin,0.7*(ymax-ymin)+ymin,"Mean: "+str(mean_diff_ra_2*3600)+" arcseconds",color = 'blue')
plt.text(0.5*(xmax-xmin)+xmin,0.8*(ymax-ymin)+ymin,"Mean: "+str(mean_diff_ra_lagr*3600)+" arcseconds",color = 'red')

#Right Ascension
#plt.plot(JD_time_a,diff_ra,'ro',JD_time_a,diff_ra_2,'bx',JD_time_a,diff_ra_3,'r+', JD_time_a, diff_ra_lagr, 'b+')
plt.plot(JD_time_a,diff_ra_2,'bx', JD_time_a, diff_ra_lagr, 'r+', [xmin,xmax], [0,0],'--')
